    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (logHeaders) {
            ...
            if (!logBody || !hasRequestBody) {
                ...
                logger.log("");
                if (isPlaintext(buffer) && !isMultipart(contentType)) {
                    if (contentType.toString().equals("application/x-www-form-urlencoded")) {
                        logger.log(URLDecoder.decode(buffer.readString(charset),"UTF-8"));
                    } else {
        ...
        if (logHeaders) {
            ...
            if (!logBody || !HttpHeaders.hasBody(response)) {
                ...
                if (contentLength != 0 && !isMultipart(contentType)) {
                    logger.log("");
